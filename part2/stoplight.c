...
int main(int argc, char * argv[]) {
    ...
    if( 0 != create_semaphores() ){
      fprintf(stderr,"Error: Creating semaphores failed.");
      return -1;
    }
    /*
     * Create Car Thread(s)
    ...
    for(i = 0; i < num_cars; ++i) {
        ...
        //printf("running for the %d th time\n", i);
    }
     ...
     * 
     */
    if( 0 != destroy_semaphores() ){
      fprintf(stderr,"Error: Destroying semaphores failed.");
      return -1;
    }
...
void *start_car(void *param) {
   ...
   // Section for car set up was already in while loop, it can be updated at the beginning of each loop instead of before, and at the end
    ...
    while( time_to_exit == FALSE ) {
        ...
        while(1){
            ...
            semaphore_wait(&conditionSignal);//t1
                ...
                if(this_car.car_id == endCarPtr.car_id && isOpen[this_car.appr_dir] == 1){
                    ...
                    semaphore_wait(&queueMutex);
                        //printSafe("Car: %d is being dequeued\n", endCarPtr.car_id);
                        //printSafe("The size of the queue is: %d\n", getQueueSize(&car_q));
                        dequeue(&car_q, &endCarPtr);
                        
                        //printSafe("Car: %d has been dequeued\n", endCarPtr.car_id);
                    semaphore_post(&queueMutex);
       ...
       error.
        ...
        So in order to implment bakers algoithm, for example on a left turn,
        We need to have three if checks, each one locking the semaphore and making
        sure that the return value is 0.
        
        If at any point we hit a return value of -1, then we must back out the previous
        mutexes that we have locked.
...
int create_semaphores(){
    ...
    if( 0 != (ret = semaphore_create(&northWest,1)) ) {
      fprintf(stderr, "Error: semaphore_create() failed with %d\n", ret);
      return -1;
    }
    if( 0 != (ret = semaphore_create(&northEast,1)) ) {
      fprintf(stderr, "Error: semaphore_create() failed with %d\n", ret);
      return -1;
    }
    if( 0 != (ret = semaphore_create(&southEast,1)) ) {
      fprintf(stderr, "Error: semaphore_create() failed with %d\n", ret);
      return -1;
    }
    if( 0 != (ret = semaphore_create(&southWest,1)) ) {
      fprintf(stderr, "Error: semaphore_create() failed with %d\n", ret);
      return -1;
    }
    if( 0 != (ret = semaphore_create(&isOpenMutex,1)) ) {
      fprintf(stderr, "Error: semaphore_create() failed with %d\n", ret);
      return -1;
    }
    if( 0 != (ret = semaphore_create(&queueMutex,1)) ) {
      fprintf(stderr, "Error: semaphore_create() failed with %d\n", ret);
      return -1;
    }
    if( 0 != (ret = semaphore_create(&conditionSignal,1)) ) {
      fprintf(stderr, "Error: semaphore_create() failed with %d\n", ret);
      return -1;
    }
    if( 0 != (ret = semaphore_create(&printStuff,1)) ) {
      fprintf(stderr, "Error: semaphore_create() failed with %d\n", ret);
      return -1;
    }
    if( 0 != (ret = semaphore_create(&statisticMutex,1)) ) {
      fprintf(stderr, "Error: semaphore_create() failed with %d\n", ret);
      return -1;
    }
    return 0;
...
int destroy_semaphores(){
    int ret;
    if( 0 != (ret = semaphore_destroy(&northWest)) ) {
        fprintf(stderr, "Error: semaphore_destroy() failed with %d\n", ret);
        return -1;
    }
    if( 0 != (ret = semaphore_destroy(&northEast)) ) {
        fprintf(stderr, "Error: semaphore_destroy() failed with %d\n", ret);
        return -1;
    }
    if( 0 != (ret = semaphore_destroy(&southEast)) ) {
        fprintf(stderr, "Error: semaphore_destroy() failed with %d\n", ret);
        return -1;
    }
    if( 0 != (ret = semaphore_destroy(&southWest)) ) {
        fprintf(stderr, "Error: semaphore_destroy() failed with %d\n", ret);
        return -1;
    }
     if( 0 != (ret = semaphore_destroy(&isOpenMutex)) ) {
        fprintf(stderr, "Error: semaphore_destroy() failed with %d\n", ret);
        return -1;
    }
     if( 0 != (ret = semaphore_destroy(&queueMutex)) ) {
        fprintf(stderr, "Error: semaphore_destroy() failed with %d\n", ret);
        return -1;
    }
    if( 0 != (ret = semaphore_destroy(&conditionSignal)) ) {
        fprintf(stderr, "Error: semaphore_destroy() failed with %d\n", ret);
        return -1;
    }
    if( 0 != (ret = semaphore_destroy(&printStuff)) ) {
        fprintf(stderr, "Error: semaphore_destroy() failed with %d\n", ret);
        return -1;
    }
     if( 0 != (ret = semaphore_destroy(&statisticMutex)) ) {
        fprintf(stderr, "Error: semaphore_destroy() failed with %d\n", ret);
        return -1;
    }
    return 0;
